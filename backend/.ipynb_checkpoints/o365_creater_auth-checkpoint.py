...
class pwd():
    ...
    async def CAPTCHA_verify_api_check(self,CAPTCHA,jsfuncstr,use_real=True):
        ...
        try:
            ...
            prcs = multiprocessing.Process(target=self.check_func_wrapper, args=[check_func,response,func_ret])
            prcs.start()
    ...
    def setProperty(self,sid,key,val):
        ...
        with open(self.config_path,"w") as config:
            config.write(json.dumps(self.__dict__,ensure_ascii=False,indent = 2,default=lambda o:None))
    async def get_pwd_with_CAPTCHA_check(self,email_in,CAPTCHA,appName="Example Organization"):
        ...
        ret = self.get_pwd(email_in,CAPTCHA,appName)
        if self.CAPTCHA_enable == True:
    ...
    def get_pwd(self,email_in,CAPTCHA,appName="Example Organization"):
...
class pwd_guest(pwd):
    ...
    def get_pwd(self,email_in,CAPTCHA,appName="Example Organization"):
        ...
        email_html = self.MAIL_msg_cont.replace("__new_code_here__",new_pwd).replace("__verify_email_here__",email_in).replace("__app_name_here__",appName)
        email_text = "\n".join(filter(None,map((lambda x:x[max(0,re.search(r'[^ ]', x).start() if re.search(r'[^ ]', x) != None else len(x)):]),re.sub(r'<.*?>',"", email_html).split("\n"))))
